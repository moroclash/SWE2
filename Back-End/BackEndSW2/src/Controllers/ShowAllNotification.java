...
public class ShowAllNotification implements Initializable, buttonableInter {
	...
	public void initialize(URL url, ResourceBundle rb) {
		...
		ConsumerAccount acc= (ConsumerAccount) Data.ExistAcc;
		HashSet<AccNotification> notif = (HashSet<AccNotification>) acc.getNotifications();
		// NEED some Abstruction in here,
		// Some helper functions.
		// To clean this duplication

	notif.forEach(notyfy->{
		...
		notificationFromName1.setText(notyfy.getFromAccount_id().getUserName());
		...
		notificationMessage1.setText(notyfy.getMassage());
		...
		notificationTime1.setText(notyfy.getDate().toString());
